...
def main():
    ...
    # for bin_seq in bin_seq_paths:
    #     sample_name = bin_seq.split('/')[-1].replace(f'cpm{cpm}_mutation_analysis.csv','')
    #     metadata_sample = metadata_df[metadata_df['sample_id'].str.contains(sample_name)].copy()
    #     try:
    #         num_replicate[sample_name] = metadata_sample['replicate'].tolist()[0]
    #     except Exception as e:
    #         print(f'cell population {sample_name} and bin not found in metadata! \n{e}')
    #     try:
    #         bin_num = metadata_sample['bin']
    #         bin_declare[sample_name] = bin_num.tolist()[0]
    #     except Exception as e:
    #         print(f'cell population {sample_name} and bin not found in metadata! \n{e}')
    # print(num_replicate)
    # print(bin_declare)
    ...
    for replicate in replicate_declare_dict.keys():
        ...
        dfs_rep_dict = {}
        for binnum, sample in enumerate(bin_list):
            dfs_rep_dict[f'rep{replicate} bin {binnum}'] = pd.read_csv(sample,index_col=0)
        dfs_rep_dict_agg = {}
        for bin_df in dfs_rep_dict.keys():
            dfs_rep_dict_agg[f'rep{replicate} bin {binnum}'] = aggregate_by_aa_sequence(dfs_rep_dict[f'rep{replicate} bin {binnum}'])
        concat_list = [repbin_df for k, repbin_df in dfs_rep_dict_agg.items() if str(replicate) in str(k)]
        dfs_concat_master[f'rep{replicate}_concat'] = pd.concat(concat_list).reset_index(drop=True).drop(['read_count'], axis=1)
        dfs_concat_master[f'rep{replicate}_concat'] = dfs_concat_master[f'rep{replicate}_concat'].drop_duplicates(subset=['aa_sequence'],).reset_index(drop=True)   
        dfs_concat_master[f'rep{replicate}_concat'] = dfs_concat_master[f'rep{replicate}_concat'].assign(bin_1=0, bin_2=0, bin_3=0, bin_4=0)
   
    
    
    print(dfs_concat_master)
    


    
    # print(dfs_rep_dict.keys())
    # print(next(iter(dfs_rep_dict_agg.keys())))
    # print(next(iter(dfs_rep_dict_agg.values())))
