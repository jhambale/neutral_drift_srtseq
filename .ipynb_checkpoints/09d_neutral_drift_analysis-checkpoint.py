...
def main():
    ...
    med_rep1_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/20mdbindcpm{cpm}_mutation_analysis.csv'
    med_rep2_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/21mdbindcpm{cpm}_mutation_analysis.csv'
    med_rep3_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/15mdbindcpm{cpm}_mutation_analysis.csv'

    low_rep1_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/20lobindcpm{cpm}_mutation_analysis.csv'
    low_rep2_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/21lobindcpm{cpm}_mutation_analysis.csv'
    low_rep3_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/15lobindcpm{cpm}_mutation_analysis.csv'

    hi_rep1_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/20hibindcpm{cpm}_mutation_analysis.csv'
    hi_rep2_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/21hibindcpm{cpm}_mutation_analysis.csv'
    hi_rep3_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/15hibindcpm{cpm}_mutation_analysis.csv'

    no_rep1_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/20nobindcpm{cpm}_mutation_analysis.csv'
    no_rep2_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/21nobindcpm{cpm}_mutation_analysis.csv'
    no_rep3_path = f'../../minibinders_orthorep_data/minibinders_orthorep_outputs/{expt_id}/mutation_dfs/15nobindcpm{cpm}_mutation_analysis.csv'
    ...
    med_rep2_df = aggregate_by_aa_sequence(med_rep2_df)
    med_rep3_df = aggregate_by_aa_sequence(med_rep3_df)

    low_rep1_df = aggregate_by_aa_sequence(low_rep1_df)
    low_rep2_df = aggregate_by_aa_sequence(low_rep2_df)
    low_rep3_df = aggregate_by_aa_sequence(low_rep3_df)

    hi_rep1_df = aggregate_by_aa_sequence(hi_rep1_df)
    hi_rep2_df = aggregate_by_aa_sequence(hi_rep2_df)
    hi_rep3_df = aggregate_by_aa_sequence(hi_rep3_df)

    no_rep1_df = aggregate_by_aa_sequence(no_rep1_df)
    no_rep2_df = aggregate_by_aa_sequence(no_rep2_df)
    no_rep3_df = aggregate_by_aa_sequence(no_rep3_df)
    ...
    master_rep1_df = pd.concat([med_rep1_df,
                               low_rep1_df,
                               hi_rep1_df,
                               no_rep1_df]).reset_index(drop=True).drop(['read_count'], axis=1)

    master_rep2_df = pd.concat([med_rep2_df,
                               low_rep2_df,
                               hi_rep2_df,
                               no_rep2_df]).reset_index(drop=True).drop(['read_count'], axis=1)
    
    master_rep3_df = pd.concat([med_rep3_df,
                               low_rep3_df,
                               hi_rep3_df,
                               no_rep3_df]).reset_index(drop=True).drop(['read_count'], axis=1)

    # drop duplicate rows with the same amino acid sequence
    master_rep1_df = master_rep1_df.drop_duplicates(['aa_sequence']).reset_index(drop=True)
    master_rep2_df = master_rep2_df.drop_duplicates(['aa_sequence']).reset_index(drop=True)
    master_rep3_df = master_rep3_df.drop_duplicates(['aa_sequence']).reset_index(drop=True)
    ...
    master_rep3_df = master_rep3_df.assign(bin_1=0, bin_2=0, bin_3=0, bin_4=0)
    ...
    no_rep3_counts_dict = dict(zip(no_rep3_df['aa_sequence'],no_rep3_df['read_count']))

    low_rep1_counts_dict = dict(zip(low_rep1_df['aa_sequence'],low_rep1_df['read_count']))
    low_rep2_counts_dict = dict(zip(low_rep2_df['aa_sequence'],low_rep2_df['read_count']))
    low_rep3_counts_dict = dict(zip(low_rep3_df['aa_sequence'],low_rep3_df['read_count']))

    med_rep1_counts_dict = dict(zip(med_rep1_df['aa_sequence'],med_rep1_df['read_count']))
    med_rep2_counts_dict = dict(zip(med_rep2_df['aa_sequence'],med_rep2_df['read_count']))
    med_rep3_counts_dict = dict(zip(med_rep3_df['aa_sequence'],med_rep3_df['read_count']))
    ...
    hi_rep3_counts_dict = dict(zip(hi_rep3_df['aa_sequence'],hi_rep3_df['read_count']))
    ...
    raw_counts_rep3_df = retrieve_filter_counts(master_rep3_df,
                               no_rep3_counts_dict,
    ...
    norm_counts_rep1_df = raw_counts_rep1_df.copy()

    # norm_counts_rep2_df = raw_counts_rep2_df.copy().iloc[:, :first_n_columns]
    norm_counts_rep2_df = raw_counts_rep2_df.copy()
    
    # norm_counts_rep3_df = raw_counts_rep3_df.copy().iloc[:, :first_n_columns]
    norm_counts_rep3_df = raw_counts_rep3_df.copy()
    ...
    raw_counts_only_rep3_df = raw_counts_rep3_df.copy().iloc[:,-4:]
    ...
    # normalize each row by the sum of each row
    norm_counts_rep3_df.iloc[:, -4:] = (
    norm_counts_rep3_df.iloc[:, -4:]
    .div(norm_counts_rep3_df.iloc[:, -4:].sum(axis=1), axis=0)
    )

    norm_counts_rep3_df['normalized_score_rep3'] = (norm_counts_rep3_df['bin_1']*0 \
    ...
    # Get the last column of norm_counts_rep2_df and norm_counts_rep3_df
    # last_columns = [norm_counts_rep2_df[["aa_sequence", "normalized_score_rep2"]], norm_counts_rep3_df[["aa_sequence", "normalized_score_rep3"]]] 
    last_column_rep2 = norm_counts_rep2_df[["aa_sequence", "normalized_score_rep2"]]
    last_column_rep3 = norm_counts_rep3_df[["aa_sequence", "normalized_score_rep3"]]


    # Merge norm_counts_rep1_df with the last column from norm_counts_rep2_df based on dna_sequence
    norm_counts_df = norm_counts_rep1_df.copy().merge(last_column_rep2, on="aa_sequence", how="inner").merge(last_column_rep3, on="aa_sequence", how="inner")
    ...
    ax.scatter(
        ...
        norm_counts_annotated_df['normalized_score_rep3'],
    ...
    ax.scatter(
        ...
        norm_counts_annotated_df['normalized_score_rep3'][parent_mask],
        c='orange',
        edgecolors='black',
        s=750,
        linewidth=2,
    ...
    ax.set_zlabel('Normalized Score Replicate 3', fontsize=16)
    ...
    rep3_scores = norm_counts_annotated_df['normalized_score_rep3']
    ...
    textstr = (f"Pearson's r (rep 1 and rep 2): {correlation_coefficient:.2f}, p = {p_value:.2e}\n"
               f"Spearman's r (rep 1 and rep 2): {correlation_coefficient_spear:.2f}, p = {p_value_spear:.2e}")
    plt.gca().text(-0.01, 0.01, 0.5, textstr, transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    out_name = out_dir + f'library_scores_{expt_id}_d.csv'
    out_name_fig = out_dir_figs + f'library_scores_{expt_id}'

    # save plot
    plt.savefig(out_name_fig+'.png', bbox_inches='tight', dpi=400)
    plt.savefig(out_name_fig+'.pdf', bbox_inches='tight', dpi=400)
